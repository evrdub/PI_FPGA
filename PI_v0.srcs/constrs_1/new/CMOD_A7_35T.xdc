## Ce fichier est un .xdc général pour le CmodA7 rev.B
## Pour l’utiliser dans un projet :
## - décommenter les lignes correspondant aux broches utilisées
## - renommez les ports utilisés (dans chaque ligne, après get_ports) en fonction des noms de signaux de niveau supérieur dans le projet

# Signal d’horloge 12 MHz
...
# Boutons
#set_property -dict { PACKAGE_PIN A18   IOSTANDARD LVCMOS33 } [get_ports { RESETN }]; #IO_L19N_T3_VREF_16 Sch=btn[0]
...
# Broches XADC analogiques
# Ne les déclarez que si vous souhaitez utiliser les broches 15 et 16 comme entrées analogiques à extrémité unique. broche 15 -> vaux4, broche16 -> vaux12
...
## Broches GPIO
## Les broches 15 et 16 doivent rester commentées si elles sont utilisées comme entrées analogiques
